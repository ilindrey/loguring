...
IH_T = TypeVar("IH_T", bound="InterceptHandler")
C_T = TypeVar("C_T", bound="Configure")
...
class InterceptHandler(logging.Handler):
    ...
    def __init__(self: IH_T, loguru_logger: Logger, *args: Any, **kwargs: Any) -> None:
        self._logger = loguru_logger
        super().__init__(*args, **kwargs)

    def emit(self: IH_T, record: logging.LogRecord) -> None:
...
class ConfigureLogger:
    ...
    def initialize(self: C_T, override_loggers: Optional[Iterable[str]] = None) -> None:
        ...
        # to correctly identify module, file, func, line and other message call parameters
        logging.basicConfig(
    ...
    def _find_logging_loggers(
        self: C_T, logger_tree: Tuple, override_loggers: Set[str]
    ) -> None:
        ...
        for standard_logger in logger_tree[2]:
            ...
            self._find_logging_loggers(standard_logger, override_loggers)  # type: ignore

    def _get_logging_config(
        self: C_T, override_loggers: Iterable[str]
    ) -> Dict[str, Any]:
        return {
            "version": 1,
    ...
    def _override_logging_config(self: C_T, override_loggers: Iterable[str]) -> None:
        config = self._get_logging_config(override_loggers)
        dictConfig(config)  # type: ignore

    def _get_logger_handlers(self: C_T) -> List[Dict[str, Any]]:
        return [
            {
                "sink": stdout,
                "level": self._level,
    ...
    def _configure_logger(self: C_T) -> None:
